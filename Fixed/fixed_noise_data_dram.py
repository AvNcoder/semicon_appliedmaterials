...
# =============================================================================
# 1. REALISTIC 6F^2 DRAM CELL & DIE LAYOUT GENERATOR
# =============================================================================

def generate_dram_6f2_cell(unit_size: int = 400) -> np.ndarray:
    """
    Generates a 6F^2 DRAM unit cell based on TI & Hynix design rules.
    Outputs a float32 array in [0.0, 255.0].
    """
    cell = np.full((unit_size, unit_size), 35.0, dtype=np.float32)  # STI Substrate

    y_grid, x_grid = np.ogrid[:unit_size, :unit_size]

    # 1. Oblique Active Moats (Silicon Active Islands)
    moat_mask = np.abs((x_grid - 0.4 * y_grid - 40) % unit_size - unit_size // 2) < (unit_size * 0.12)
    cell[moat_mask] = 100.0  

    # 2. Straight Wordlines (Buried Gate)
    ...
    # 3. Wavy / Arcuate Bitline
    wave = (unit_size * 0.08) * np.sin(2 * np.pi * y_grid / unit_size)
    bl_mask = np.abs((x_grid + wave - unit_size // 2) % unit_size) < (unit_size * 0.05)
    cell[bl_mask] = 210.0  

    # 4. Storage Node Contacts
    cx1, cy1 = int(unit_size * 0.25), int(unit_size * 0.25)
    cx2, cy2 = int(unit_size * 0.75), int(unit_size * 0.75)
    r = max(2, int(unit_size * 0.06))

    contact_mask = ((x_grid - cx1)**2 + (y_grid - cy1)**2 <= r**2) | \
                   ((x_grid - cx2)**2 + (y_grid - cy2)**2 <= r**2)
    cell[contact_mask] = 255.0  

    return cell


def generate_master_dram_die(master_size: int = 10000, cell_pitch: int = 400) -> np.ndarray:
    """Tiles the 6F^2 DRAM cell across a large master die field."""
    unit_tile = generate_dram_6f2_cell(unit_size=cell_pitch)
...
# =============================================================================
# 3. SEM NOISE & ARTIFACT PRIMITIVES
# =============================================================================

def apply_poisson_gaussian_noise(img: np.ndarray, shot_scale: float = 1.0, readout_sigma: float = 6.0) -> np.ndarray:
    """Models secondary electron shot noise (Poisson) and detector amplifier noise (Gaussian)."""
...
def apply_edge_brightening(img: np.ndarray, intensity: float = 0.5) -> np.ndarray:
    """Simulates top-down SEM edge blooming / primary electron scattering near sidewalls."""
...
def apply_astigmatism_blur(img: np.ndarray, sigma_x: float = 1.1, sigma_y: float = 0.55, angle_deg: float = 30.0) -> np.ndarray:
    """Simulates anisotropic focus blur due to quadrupole beam distortion."""
    img_32f = img.astype(np.float32, copy=False)
...
def apply_line_scan_jitter(img: np.ndarray, max_shift: int = 2, jitter_prob: float = 0.08) -> np.ndarray:
    """Simulates raster line sync jitter and electromagnetic pickup."""
    output = img.copy()
    h, w = output.shape[:2]
    for r in range(h):
...
def apply_stage_drift(img: np.ndarray, max_drift: float = 1.5) -> np.ndarray:
    """Simulates sub-pixel piezo stage creep / thermal drift during raster scanning."""
    img_32f = img.astype(np.float32, copy=False)
    h, w = img_32f.shape[:2]
...
def apply_charging_and_streaks(img: np.ndarray, intensity: float = 18.0) -> np.ndarray:
    """Simulates localized dielectric surface charging and secondary electron deflection streaks."""
    img_32f = img.astype(np.float32, copy=False)
    h, w = img_32f.shape[:2]
...
def apply_ac_mains_ripple(img: np.ndarray, amplitude: float = 3.0, freq_cycles: float = 6.0) -> np.ndarray:
    """Simulates 50/60 Hz power supply noise ripple superimposed on vertical scan."""
    h, w = img.shape[:2]
...
def apply_hydrocarbon_deposition(img: np.ndarray, dark_attenuation: float = 0.08) -> np.ndarray:
    """Simulates beam-induced dark hydrocarbon contamination buildup over scanned areas."""
    img_32f = img.astype(np.float32, copy=False)
    h, w = img_32f.shape[:2]
...
def tonemap_to_uint8(img_float: np.ndarray) -> np.ndarray:
    """Maps floating point intensity levels into standard 8-bit uint8 grayscale [0, 255]."""
...
# =============================================================================
# 4. CALIBRATED NOISE STACKS
# =============================================================================

def apply_light_reference_noise(img: np.ndarray) -> np.ndarray:
    """Applies high-resolution SEM acquisition noise to the Reference crop."""
    output = img.copy()
...
def apply_calibrated_search_noise(img: np.ndarray) -> np.ndarray:
    """Applies realistic wide-field inspection SEM artifacts to Search die image."""
    output = img.copy()
...
# =============================================================================
# 5. DATASET GENERATOR WITH GT TRACKING
# =============================================================================

def generate_dram_sem_dataset(output_dir: str, num_samples: int = 30):
    """
    Generates paired Reference + Search SEM images for DRAM 6F^2 layout 
    with 10x downsampling embedding, geometric jitter, and ground-truth JSONs.
    """
    ...
    print("Generating Master 6F^2 DRAM Layout Die Field...")
    master_die = generate_master_dram_die(master_size=MASTER_SIZE, cell_pitch=CELL_PITCH)

    # Clean wide base image (25x25 cell array sampled down)
    ...
    base_dram = np.array(wide_base_pil, dtype=np.float32)

    for idx in range(1, num_samples + 1):
        np.random.seed(idx * 4096)

        # 1. High-mag Reference (1000x1000 crop from Master)
        ...
        clean_ref_1000 = add_unique_local_features(clean_ref_1000, seed=idx * 777)

        # 2. Base Search canvas
        clean_search_canvas = base_dram.copy()

        # 3. Random Ground Truth location for the embedded patch
        center_x = float(np.random.uniform(170.0, search_size - 170.0))
        center_y = float(np.random.uniform(170.0, search_size - 170.0))

        # 4. Rotation and scale jitter on embedded reference
        ...
        # 5. Embed patch into Wide Search canvas
        clean_search_canvas[y1:y2, x1:x2] = clean_ref_100

        # 6. Apply realistic physical SEM noise stacks
    ...
    print(f"Successfully generated {num_samples} DRAM SEM Drift-Sense pairs in: {output_dir}")


if __name__ == "__main__":
    OUTPUT_PATH = r"./Fixed/data_2"
    generate_dram_sem_dataset(output_dir=OUTPUT_PATH, num_samples=100)