...
# =============================================================================
# 1. REALISTIC 6F² DRAM CELL & PATTERN GENERATOR
# =============================================================================

def generate_dram_6f2_cell(unit_size: int = 40) -> np.ndarray:
    """Generates a realistic 6F² DRAM unit cell."""
    cell = np.full((unit_size, unit_size), 35.0, dtype=np.float32)
    y_grid, x_grid = np.ogrid[:unit_size, :unit_size]
    
    # Oblique Active Moats
    moat_mask = np.abs((x_grid - 0.4 * y_grid - 0.1 * unit_size) % unit_size - unit_size // 2) < (unit_size * 0.12)
    cell[moat_mask] = 100.0
    
    # Straight Wordlines
    wl_y1, wl_y2 = int(unit_size * 0.3), int(unit_size * 0.7)
    wl_thick = max(1, int(unit_size * 0.08))
    cell[max(0, wl_y1 - wl_thick):min(unit_size, wl_y1 + wl_thick), :] = 160.0
    cell[max(0, wl_y2 - wl_thick):min(unit_size, wl_y2 + wl_thick), :] = 160.0
...
def generate_clean_dram_6f2_pattern(height: int = 1000, width: int = 1000, unit_size: int = 40) -> np.ndarray:
    unit_tile = generate_dram_6f2_cell(unit_size=unit_size)
    reps_y = height // unit_size
    reps_x = width // unit_size
    return np.tile(unit_tile, (reps_y, reps_x)).astype(np.float32)


def add_unique_local_features(base_pattern: np.ndarray, seed: int) -> np.ndarray:
    """Unique macro features sized to survive 10× downsampling."""
    ...
    for _ in range(num_breaks):
        ...
        pattern[by:by+bh, bx:bx+bw] = 35.0
    ...
    for _ in range(num_pads):
        ...
        cv2.circle(pattern, (cx, cy), r // 2, 50.0, -1)
    ...
    cv2.line(pattern, (lx1, ly1), (lx2, ly2), 210.0, thickness=rng.integers(12, 20))
...
# =============================================================================
# 4. MILD NOISE STACKS (consistent with other styles)
# =============================================================================
...
def apply_calibrated_search_noise(img: np.ndarray) -> np.ndarray:
    """Mild noise – pattern and embedded patch stay clearly visible."""
    output = img.copy()
...
# =============================================================================
# 5. PAIRED DATASET GENERATOR
# =============================================================================

def generate_6f2_sem_dataset(output_dir: str, num_samples: int = 30):
    ...
    base_dram = generate_clean_dram_6f2_pattern(search_size, search_size, unit_size=40)
    
    for idx in range(1, num_samples + 1):
        np.random.seed(idx * 101)
        
        # High-mag Reference + unique features
        clean_ref_1000 = add_unique_local_features(base_dram, seed=idx * 999)
        
        # 10× downsampled patch
        clean_ref_100 = cv2.resize(clean_ref_1000, (embedded_size, embedded_size), interpolation=cv2.INTER_AREA)
        
        # Pure periodic Search background
    ...
    print(f"Successfully generated {num_samples} usable 6F² DRAM Drift-Sense pairs in: {output_dir}")


if __name__ == "__main__":
    OUTPUT_PATH = r"./Fixed/data_2"
    generate_6f2_sem_dataset(output_dir=OUTPUT_PATH, num_samples=100)