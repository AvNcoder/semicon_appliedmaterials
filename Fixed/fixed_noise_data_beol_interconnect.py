...
def add_unique_local_features(base_pattern: np.ndarray, seed: int) -> np.ndarray:
    """Adds unique macro features only to the high-mag Reference so they survive 10× downsampling."""
    rng = np.random.default_rng(seed)
    pattern = base_pattern.copy()
    h, w = pattern.shape
    
    # Local metal cuts / breaks
    num_breaks = rng.integers(2, 4)
    for _ in range(num_breaks):
        bx = rng.integers(180, w - 220)
        by = rng.integers(180, h - 220)
        bw, bh = rng.integers(50, 100), rng.integers(40, 80)
        pattern[by:by+bh, bx:bx+bw] = 15.0
        
    # Distinctive alignment pads
    num_pads = rng.integers(2, 3)
    for _ in range(num_pads):
        cx = rng.integers(220, w - 220)
        cy = rng.integers(220, h - 220)
        r = rng.integers(40, 65)
        cv2.circle(pattern, (cx, cy), r, 245.0, -1)
        cv2.circle(pattern, (cx, cy), r // 2, 30.0, -1)
        
    # Macro interconnect line
    lx1, ly1 = rng.integers(120, w-120), rng.integers(120, h-120)
    lx2, ly2 = rng.integers(120, w-120), rng.integers(120, h-120)
    cv2.line(pattern, (lx1, ly1), (lx2, ly2), 200.0, thickness=rng.integers(12, 20))
...
def generate_beol_sem_dataset(output_dir: str, num_samples: int = 30):
    ...
    base_beol = np.array(wide_base_pil, dtype=np.float32)
    ...
    for idx in range(1, num_samples + 1):
        ...
        gt_data = {
            "sample_id": idx,
            "style": "dram_octagonal",   # (or dram_6f2 / finfet_sram / beol_interconnect, unchanged)
...
if __name__ == "__main__":
    ...
    generate_beol_sem_dataset(output_dir=OUTPUT_PATH, num_samples=100)