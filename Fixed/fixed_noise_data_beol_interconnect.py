...
# =============================================================================
# 1. REALISTIC BEOL DUAL-LAYER INTERCONNECT CELL & DIE LAYOUT GENERATOR
# =============================================================================

def generate_beol_interconnect_cell(unit_size: int = 400) -> np.ndarray:
    """Generates an advanced BEOL dual-layer routing pattern."""
    cell = np.full((unit_size, unit_size), 15.0, dtype=np.float32)
    
    y_grid, x_grid = np.ogrid[:unit_size, :unit_size]
    
    # Metal 1 (M1) Lower Horizontal Tracks
    m1_pitch = unit_size // 5
    m1_thick = max(1, int(unit_size * 0.04))
    for m in range(5):
        m1_center = m * m1_pitch + m1_pitch // 2
        cell[max(0, m1_center - m1_thick):min(unit_size, m1_center + m1_thick), :] = 100.0
        
    # Metal 2 (M2) Upper Vertical Tracks
    m2_pitch = unit_size // 5
    m2_thick = max(1, int(unit_size * 0.04))
    for m in range(5):
        m2_center = m * m2_pitch + m2_pitch // 2
        cell[:, max(0, m2_center - m2_thick):min(unit_size, m2_center + m2_thick)] = 170.0
        
    # Self-Aligned Vias (checkerboard)
...
def generate_master_beol_die(master_size: int = 10000, cell_pitch: int = 400) -> np.ndarray:
    """Tiles the BEOL cell across a large master die."""
    unit_tile = generate_beol_interconnect_cell(unit_size=cell_pitch)
...
# =============================================================================
# 3. SEM NOISE PRIMITIVES (mild usage)
# =============================================================================
...
def apply_calibrated_search_noise(img: np.ndarray) -> np.ndarray:
    """Mild noise that keeps BEOL grid + embedded patch clearly visible."""
    output = img.copy()
...
def generate_beol_sem_dataset(output_dir: str, num_samples: int = 30):
    """
    ...
    print("Building master BEOL die...")
    master_die = generate_master_beol_die(master_size=MASTER_SIZE, cell_pitch=CELL_PITCH)
    
    # Clean 1000×1000 periodic base
    base_beol = cv2.resize(master_die, (search_size, search_size), interpolation=cv2.INTER_AREA)


    for idx in range(1, num_samples + 1):
        np.random.seed(idx * 4048)
        
        # 1. High-mag Reference (random 1000×1000 crop) + unique features
        ...
        # 3. Search starts as pure periodic BEOL
        clean_search_canvas = base_beol.copy()
        
        # 4. Random GT center
        center_x = float(np.random.uniform(170.0, search_size - 170.0))
        center_y = float(np.random.uniform(170.0, search_size - 170.0))
        _search_cell = CELL_PITCH / 10.0
        center_x = round(center_x / _search_cell) * _search_cell
        center_y = round(center_y / _search_cell) * _search_cell
...
# True geometric center of the patch (removes the 0.5 px floor-division bias)
        ...
        _feather = 4
        _blend = np.ones((jittered_size, jittered_size), dtype=np.float32)
        _ramp = np.linspace(0, 1, _feather, dtype=np.float32)
        _blend[:_feather, :] *= _ramp[:, None]
        _blend[-_feather:, :] *= _ramp[::-1][:, None]
        _blend[:, :_feather] *= _ramp[None, :]
        _blend[:, -_feather:] *= _ramp[::-1][None, :]
        _region = clean_search_canvas[y1:y2, x1:x2]
        clean_search_canvas[y1:y2, x1:x2] = clean_ref_100 * _blend + _region * (1 - _blend)
        ...
        gt_data = {
            "sample_id": idx,
            "style": "beol_interconnect",   # (or dram_6f2 / finfet_sram / beol_interconnect, unchanged)
    ...
    print(f"Successfully generated {num_samples} usable BEOL Drift-Sense pairs in: {output_dir}")
