...
# =============================================================================
# 1. BASE DRAM PATTERN GENERATOR (Octagonal Contacts)
# =============================================================================

def generate_clean_dram_pattern(height: int = 1000, width: int = 1000, unit_size: int = 20) -> np.ndarray:
    """Generates clean base DRAM layout with octagonal contact pads."""
    tile = np.full((unit_size, unit_size), 38.0, dtype=np.float32)
    
    half = unit_size // 2
    line_half_width = 2
    
    # Wordlines & Bitlines
    tile[half - line_half_width : half + line_half_width, :] = 130.0
    tile[:, half - line_half_width : half + line_half_width] = 130.0
    
    # Octagonal Contact Pads
    radius = 5.0
    angles = np.linspace(0, 2 * np.pi, 9)[:-1] + (np.pi / 8)
    octagon_pts = np.array([
        [int(round(half + radius * np.cos(a))), int(round(half + radius * np.sin(a)))]
        for a in angles
    ], dtype=np.int32)
    
    via_mask = np.zeros((unit_size, unit_size), dtype=np.uint8)
    cv2.fillConvexPoly(via_mask, octagon_pts, 1)
    tile[via_mask == 1] = 225.0

    reps_y = height // unit_size
    reps_x = width // unit_size
    return np.tile(tile, (reps_y, reps_x)).astype(np.float32)


def add_unique_local_features(base_pattern: np.ndarray, seed: int) -> np.ndarray:
    """Adds unique macro features that survive 10× downsampling."""
    ...
    for _ in range(num_breaks):
        ...
        pattern[by:by+bh, bx:bx+bw] = 38.0
...
def apply_calibrated_search_noise(img: np.ndarray) -> np.ndarray:
    """Mild noise that keeps the octagonal DRAM grid + embedded patch visible."""
    output = img.copy()
...
def generate_sem_dataset(output_dir: str, num_samples: int = 30):
    """
    Generates paired Reference + Search images with 10× embedding and GT centers.
    Matches the visual style of the provided sem_ref_100x and wide samples.
    """
    ...
    base_dram = generate_clean_dram_pattern(search_size, search_size, unit_size=20)
    
    for idx in range(1, num_samples + 1):
        np.random.seed(idx * 101)
        
        # 1. High-mag Reference (1000×1000) with unique features
        clean_ref_1000 = add_unique_local_features(base_dram, seed=idx * 999)
        
        # 2. 10× downsampled patch to embed
        clean_ref_100 = cv2.resize(clean_ref_1000, (embedded_size, embedded_size), interpolation=cv2.INTER_AREA)
        
        # 3. Pure periodic Search background
    ...
    print(f"Successfully generated {num_samples} usable octagonal DRAM Drift-Sense pairs in: {output_dir}")


if __name__ == "__main__":
    OUTPUT_PATH = r"./Fixed/data_1"
    generate_sem_dataset(output_dir=OUTPUT_PATH, num_samples=100)