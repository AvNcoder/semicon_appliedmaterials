...
# =============================================================================
# 1. REALISTIC DRAM PATTERN & DIE LAYOUT GENERATOR
# =============================================================================

def generate_dram_cell(unit_size: int = 100) -> np.ndarray:
    """Generates a realistic 2D DRAM unit cell pattern."""
    cell = np.full((unit_size, unit_size), 25.0, dtype=np.float32)
    
    # Horizontal Wordline (intensity ~140)
    cell[40:60, :] = 140.0
    
    # Vertical Bitline (intensity ~160)
    cell[:, 40:60] = 160.0
    
    # Via/Contact circle at intersection (intensity ~255)
...
def generate_master_dram_die(master_size: int = 10000, cell_pitch: int = 100) -> np.ndarray:
    """Tiles the DRAM cell across a large master die."""
    unit_tile = generate_dram_cell(unit_size=cell_pitch)
...
def add_unique_local_features(base_pattern: np.ndarray, seed: int) -> np.ndarray:
    """Sized proportionally to this style's cell_pitch=100. The original
    r=40-65/thickness=12-20 were sized for the pre-rewrite unit_size=20 grid;
    at unit_size=100 they shrink too far after the 10x downsample and let
    periodic aliasing outscore the true match (verified: 40% success,
    371.7px mean error unboosted vs 100%/0.3px boosted on a 15-sample test)."""
...
def apply_calibrated_search_noise(img: np.ndarray) -> np.ndarray:
    """Mild noise that keeps DRAM structure + embedded patch clearly visible."""
    output = img.copy()
...
def generate_dram_sem_dataset(output_dir: str, num_samples: int = 30):
    ...
    print("Building master DRAM die...")
    ...
    for idx in range(1, num_samples + 1):
        ...
        # 3. Search starts as pure periodic DRAM
    ...
    print(f"Successfully generated {num_samples} usable DRAM Drift-Sense pairs in: {output_dir}")
