...
def add_unique_local_features(base_pattern: np.ndarray, seed: int) -> np.ndarray:
    """Sized proportionally to this style's cell_pitch=400."""
...
def generate_finfet_sem_dataset(output_dir: str, num_samples: int = 30):
    ...
    base_finfet = cv2.resize(master_die, (search_size, search_size), interpolation=cv2.INTER_AREA)
    ...
    for idx in range(1, num_samples + 1):
        ...
        # 2. Search starts as pure periodic FinFET
        ...
        rc = ((jittered_size - 1) / 2.0, (jittered_size - 1) / 2.0)   # fixed: (w-1)/2, not w/2
